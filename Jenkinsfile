pipeline {
    ...
    environment {
        ...
        url = 'https://github.com/Somaya-Ayman/tttest.git' // Define repo URL here
    }
        ...
        stage('Access and Modify Running Docker Container') {
            steps {
                script {
                    ...
                    def containerId = sh(script: "ssh -i ${HOST_KEY_PATH} ${REMOTE_HOST} 'docker ps -qf \"name=user1container\"'", returnStdout: true).trim()
